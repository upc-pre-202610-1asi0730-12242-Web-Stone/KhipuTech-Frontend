...
const userRole = computed(() => {
  ...
  if (role === 'museum') return t('layout.roleAdmin')
  if (role === 'public') return t('layout.roleManager')
  return t('layout.roleVisitor')
})
...
.logout-btn-sidebar { width: 100%; padding: 10px; background: rgba(231, 76, 60, 0.3); border: none; border-radius: 8px; color: white; display: flex; align-items: center; gap: 8px; cursor: pointer; transition: all 0.3s; }
.logout-btn-sidebar:hover { background: rgba(231, 76, 60, 0.5); }
.main-content { flex: 1; margin-left: 260px; transition: margin-left 0.3s ease; min-height: 100vh; }
