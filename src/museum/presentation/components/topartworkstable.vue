<template>
  <div class="card">
    <div class="card-header">
      ...
      <h3>{{ t('museum.topArtworks.title') }}</h3>
    </div>
    <div class="table-responsive">
      <table class="data-table">
        <thead>
        <tr>
          <th>{{ t('museum.topArtworks.colArtwork') }}</th>
          <th>{{ t('museum.topArtworks.colRoom') }}</th>
          <th>{{ t('museum.topArtworks.colVisits') }}</th>
          <th>{{ t('museum.topArtworks.colRetention') }}</th>
          <th>{{ t('museum.topArtworks.colStatus') }}</th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="obra in obras" :key="obra.id">
          ...
          <td>
            <span :class="['status-badge', obra.estado]">
              {{ obra.estado === 'abierto' ? t('museum.topArtworks.statusOpen') : obra.estado === 'mantenimiento' ? t('museum.topArtworks.statusMaintenance') : t('museum.topArtworks.statusClosed') }}
            </span>
          </td>
        </tr>
        </tbody>
...
const { t } = useI18n()
defineProps({ obras: { type: Array, required: true } })
</script>
...
.progress-bar-mini { height: 6px; background: #27ae60; border-radius: 3px; }
