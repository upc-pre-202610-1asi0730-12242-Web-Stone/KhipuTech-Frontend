<template>
  <div class="card">
    <div class="card-header">
      ...
      <h3>{{ t('museum.alerts.activeTitle') }}</h3>
    </div>
    <div class="alertas-lista">
      ...
      <div v-if="alertas.length === 0" class="no-alertas">{{ t('museum.alerts.noAlerts') }}</div>
    </div>
  </div>
</template>
...
const { t } = useI18n()
defineProps({ alertas: { type: Array, default: () => [] } })
</script>
...
.alerta-info { flex: 1; }
.alerta-info strong { display: block; color: #2c3e50; margin-bottom: 5px; }
.alerta-info p { margin: 0; font-size: 13px; color: #2c3e50; }
.alerta-info small { font-size: 11px; color: #7f8c8d; }
.no-alertas { text-align: center; padding: 40px; color: #27ae60; font-weight: 500; }
</style>
