<template>
  <div class="card">
    <div class="card-header">
      ...
      <h3>{{ t('museum.ranking.title') }}</h3>
      <div class="ranking-period">
        <button v-for="period in periods" :key="period" @click="$emit('update:period', period)" :class="{ active: currentPeriod === period }">
          {{ period }}
        </button>
      </div>
    </div>
      ...
      <div v-for="(obra, index) in rankings" :key="obra.id" class="ranking-item">
        ...
        <div class="ranking-stats">
          <div class="engagement-score">
            ...
            <span class="score-label">{{ t('museum.ranking.pts') }}</span>
          </div>
...
const { t } = useI18n()
defineProps({ rankings: { type: Array, required: true }, periods: { type: Array, required: true }, currentPeriod: { type: String, required: true } })
...
.trend-icon.down { color: #e74c3c; transform: rotate(180deg); }
</style>